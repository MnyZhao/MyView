...
public class ClearEditTextPwdBase extends EditText implements View.OnFocusChangeListener, TextWatcher {
    /**
     * 画线
     */
    ...
    public ClearEditTextPwdBase(Context context) {
        this(context, null);
    }

    public ClearEditTextPwdBase(Context context, AttributeSet attrs) {
        this(context, attrs, android.R.attr.editTextStyle);
    }

    public ClearEditTextPwdBase(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initCustomAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //右边按钮的大小dp
        rightDrawableSize = ta.getDimension(R.styleable.ClearEditTextHighlight_mClearEditText_rightDrawableSize, dip2px(context, 16));
        //左侧默认图片id
    ...
    private void updateDrawable(boolean hasFocus) {
        if (disableClean) {//pwd 密码 当删除不可用  不显示删除按钮
